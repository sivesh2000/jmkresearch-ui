...
const Page = memo(function Page() {
  ...
  const { activeSubCategories, isLoading, error, players } = useSelector((state: RootState) => state.activeSubCategories);
  const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
  const [selectedRow, setSelectedRow] = useState<any>(null);
  const [deleteRow, setDeleteRow] = useState<any>(null);
  const [modalOpen, setModalOpen] = useState(false);
  const [formData, setFormData] = useState({ cat_id: "", sub_category_name: "", status: true, id: null });
  ...
  const filterColumns: any[] = [
    ...
    { field: "cat_id", headerName: "Categories", type: 'dropdown', options: [], optionLabelField: 'name', optionValueField: 'id' },
  ...
  const fetchSubCategories = useCallback(async () => {
    try {
      ...
      toast.error("Failed to fetch subcategory/filters." + (error as any).response || "");
    }
  }, [dispatch]);
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      toast.error("Failed to delete sub category." + (err as any).response.data.message || "");
    }
  ...
  const handleAddClick = () => {
    setFormData({ cat_id: "", sub_category_name: "", status: true, id: null });
    setIsEdit(false);
    setModalOpen(true);
  };
  ...
  const DeleteDialog = () => {
    return (<Dialog open={deleteDialogOpen} onClose={handleDeleteCancel}>
      ...
      <DialogContent>
        <DialogContentText>
          Are you sure you want to delete the sub category &quot;{deleteRow?.name}
          &quot;?
        </DialogContentText>
      </DialogContent>
      <DialogActions>
      ...
      <Paper sx={{ height: "auto", width: "100%" }}>
        <Box sx={{ padding: 1, display: "flex", justifyContent: "space-between" }}>
          <Box sx={{ textAlign: "left", display: "flex", justifyContent: "center", alignItems: "center", gap: 1 }}>
            <TextField sx={{ width: '300px' }} variant="standard" placeholder="Sub Category Name" margin="normal" />
            <TextField sx={{ width: '300px' }} variant="standard" placeholder="Category" margin="normal" />
        ...
        <LazyDataGrid rows={activeSubCategories} getRowId={(row: any) => row.id} columns={columns} loading={isLoading} paginationModel={paginationModel} pageSizeOptions={[5, 10]} />
      </Paper>
      <MenuComponent />
      <SubCategoryModel />
      <DeleteDialog />
